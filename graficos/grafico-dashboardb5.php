...
sectorPieBefore = am4core.create("sectorPieBefore2", am4charts.PieChart3D);
	...
	mysqli_query($conexao,'SET character_set_results=utf8');  
		...
		$selecao=mysqli_query($conexao,"select * from avaliacao_risco_inerente where
			data between '$de' and '$ate' ");
		$registros=mysqli_fetch_array($selecao);	
		$riscos_inerentes=mysqli_num_rows($selecao);
		 
				
		$selecao2=mysqli_query($conexao,"select * from avaliacao_risco_residual where
			data between '$de' and '$ate' ");
		$registros2=mysqli_fetch_array($selecao2);	
		$riscos_residuais=mysqli_num_rows($selecao2);
		  
		$selecao3=mysqli_query($conexao,"select * from avaliacao_risco_futuro where
			data between '$de' and '$ate' ");
		$registros3=mysqli_fetch_array($selecao3);	
		$riscos_futuros=mysqli_num_rows($selecao3);  
					
			
					  
	?>	  
		  
		   {"sector":"Riscos Inerentes",
		   "value":"<?php echo $riscos_inerentes?>",
		   "colour":"<?php echo $cores[1] ?>"},  
		  
		  {"sector":"Riscos Residuais",
		   "value":"<?php echo $riscos_residuais ?>",
		   ...
		   {"sector":"Riscos Futuros",
		   "value":"<?php echo $riscos_futuros?>",
