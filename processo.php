...
<form action="processa-cadastro-processo.php" method="post">	
	...
	<input type="button" class="btn btn-primary mb-3" value="Voltar" onclick='history.go(-1)'><br>				
 	<h3 class="mb-4 mt-4 ml-4 mr-4">Processo 
	...
	$verificar_grupo=mysqli_query($conexao,"select * from grupos_permissoes WHERE codigo_menu='13' and codigo_grupo='$cod_grupo' and editar='1' ");
			...
			if($numero_grupo>=1){	
			?>		
		<a href="editar-processo.php?cod=<?php echo $codigo ?>"><img src="imgs/icone-editar.png" width="30" height="30" alt=""/></a>
		<?php } ?>			
	...
	mysqli_query($conexao,'SET character_set_results=utf8');
		...
		<div class="col-md-2 mb-4">
			<label>Data</label>
			<input type="text" class="form-control datepicker mb-4" name="cad-data" id="example" value="<?php echo $registros['data'] ?>" readonly>
		</div>
		
		<div class="col-md-5 mb-4">
			<label>Área</label>
				<input type="text" class="form-control datepicker mb-4" name="cad-data" id="example" value="<?php echo $registros['area'] ?>" readonly>
			
		</div>
		
		<div class="col-md-5 mb-4">
			<label>Processo</label>
			<input type="text" class="form-control mb-4" name="cad-processo" id="cad-processo" value="<?php echo $registros['processo'] ?>" readonly>
		</div>
		
		<div class="col-md-5 mb-4">
			<label>Descrição do Processo</label>
		
			<input type="text" class="form-control mb-4" name="cad-descricao" id="cad-descricao" value="<?php echo $registros['descricao'] ?>" readonly>
		</div>
		
		<div class="col-md-5 mb-4">
			<label>Aplicação(Qual unidade se aplica)</label>
			<input type="text" class="form-control" name="cad-processo" id="cad-processo" 
            <?php
            ...
            value="<?php echo $aplicacao2 ?>" readonly>
		</div>
