...
mysqli_query($conexao,'SET character_set_results=utf8');
	$selecao_planejamento=mysqli_query($conexao,"select * from tarefas_planejamento WHERE codigo_planejamento='$codigo'");
	$registros_planejamento=mysqli_fetch_array($selecao_planejamento);
	$numero=mysqli_num_rows($selecao_planejamento);
	
	
	$selecao_fase=mysqli_query($conexao,"select * from fases WHERE id='$codigo'");
...
<!-- Navegação !-->	
	...
	<input type="button" class="btn btn-primary mb-3" value="Voltar" onclick='history.go(-1)'><br>
 	<a href="cadastro-tarefa.php?cod=<?php echo $codigo ?>"> <img src="imgs/icone-mais.png" width="25" height="25" alt=""/> Nova Tarefa</a>		
	...
	<p><?php echo $registros_planejamento['descricao'] ?></p>				
...
mysqli_query($conexao,'SET character_set_client=utf8');
mysqli_query($conexao,'SET character_set_results=utf8');
				$selecao=mysqli_query($conexao,"select * from tarefas_planejamento WHERE codigo_planejamento='$codigo' ");
			...
			?>
				...
				<div class="col-md-4">
				<p><strong>Responsáveis pela fase <a onclick="AddResponsavel()" data-toggle="modal" data-target="#ModalAddResponsavel">+</a> </strong></p>	
					
				<table class="table">
					<?php
		$selecao_responsaveis=mysqli_query($conexao,"select * from responsaveis_fase_implementacao WHERE codigo_fase='$codigo'");
...
<script>
		...
		$.ajax({
		  ...
		  url:'funcoes/gravar-responsavel-planejamento.php',
		  success: function(retorno){
		   $('.close').trigger('click')
		  location.reload();		  
	...
	function RemoveResponsavel(variavel){
		  ...
		  url:'funcoes/excluir-responsavel-planejamento.php',
		  success: function(retorno){
		  location.reload();		  
