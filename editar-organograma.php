...
<form action="processa-editar-organograma.php" method="post">	
		...
		<div class="container-fluid">
			<div class="row mb-5" style="margin-top: -16px; ">
				<div class="col-md-12 bg-padrao position-fixed p-2 text-right" style="z-index: 999999">
					...
					<div class="row">
						<div class="col-md-9">
							<span class="text-white breadcrumb-item"><a href="dashboard.php" class="text-light">Dashboard</a> | Editar Organograma </span>
						</div>
					</div>
	...
	<input type="button" class="btn btn-primary mb-3" value="Voltar" onclick='history.go(-1)'><br>				
 	<h3 class="mb-4 mt-4 ml-4 mr-4">Cadastro Áreas da empresa</h3>			
		...
		<div class="col-md-4">
			<label>Empresa</label>
			<select class="form-control" name="esc-empresa" id="esc-empresa" disabled >
				<?php
				
				if($codigo<=3000){
				
				$selecao_empresas=mysqli_query($conexao,"select * from empresas WHERE id='$codigo' order by razao_social ASC");
				}
					
				if($codigo>=3000){
				$selecao_empresas=mysqli_query($conexao,"select * from filiais WHERE id='$codigo' order by razao_social ASC");	
				}
					
				$registros_empresas=mysqli_fetch_array($selecao_empresas);
				?>
		...
		<div class="col-md-4">
			...
			<input type="text" class="form-control" name="cad-area" id="cad-area"  required>
		</div>
...
<!-------------Contadores--------->	
	...
	<script>
		...
		$j(document).ready(function(){
			CarregarAreasMae(<?php echo $codigo ?>)
			CarregarTabela(<?php echo $codigo ?>)	
		})	
		...
		if (window.confirm("ATENÇÃO, Ao excluir uma área, o sistema deixará itens pertencentes a área excluída sem área ou Em transição, DESEJA EXCLUIR ASSIM MESMO?")) {	
